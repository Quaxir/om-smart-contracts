...
contract("SMAUGMarketPlace", async accounts => {
    ...
    it("interledgerReceive()", async () => {
        ...
        let offerID = tx.logs[1].args.offerID.toNumber()
        let offerDID = 1
        tx = await contract.submitOfferArrayExtra(offerID, [1, 1, 0, offerDID], {from: offerCreator, value: "1"})
        await contract.decideRequest(requestID, [offerID], {from: requestCreator})
        let givenNonce = 1
        let encodedPayload = "0x" + web3.eth.abi.encodeParameter("uint256", offerID).substr(2) + web3.eth.abi.encodeParameter("uint256", offerDID).substr(2)
        tx = await contract.interledgerReceive(givenNonce, encodedPayload, {from: owner})
        ...
        await contract.decideRequest(requestID, [offerID], {from: requestCreator})
        givenNonce = 1
        let givenOfferID = web3.eth.abi.encodeParameter("uint256", offerID)
        ...
        await contract.decideRequest(requestID, [offerID], {from: requestCreator})
        givenNonce = 1
        givenOfferID = web3.eth.abi.encodeParameter("uint256", 999)
        ...
        tx = await contract.submitOfferArrayExtra(offerID, [1, 1, 0, 1], {from: offerCreator, value: "1"})
        givenNonce = 1
        givenOfferID = web3.eth.abi.encodeParameter("uint256", offerID)
        tx = await contract.interledgerReceive(givenNonce, givenOfferID, {from: owner})
        txStatusCode = tx.logs[0].args.status
        generatedEventType = tx.logs[1].event
        eventGeneratedNonce = tx.logs[1].args.nonce
